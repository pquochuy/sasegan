...
class SEGAN(Model):
    ...
    def __init__(self, sess, args, devices, infer=False, name='SEGAN'):
        ...
        # encoder: [16, 32, 32, 64, 64, 128, 128, 256, 256, 512, 1024] (the last layer is the bottleneck)
        # decoder: [512, 256, 256, 128, 128, 64, 64, 32, 32, 16, 1] (the last layer is channel-wise convolution)
        # indices of encoder's self-attention conv layers
        self.enc_att_layer_ind = list(map(int, args.att_layer_ind.split(",")))
        # indices of decoder's self-attention conv layers
        self.dec_att_layer_ind = [(len(self.g_enc_depths) - ind - 2) for ind in self.enc_att_layer_ind if ind < (len(self.g_enc_depths) - 1)]
        ...
        try:
            while not coord.should_stop():
                ...
                if (counter / num_devices) % config.save_freq == 0:
                    ...
                    # i do not want to log too many, start log wave file after 40000 steps
                    if(counter > 200000):
