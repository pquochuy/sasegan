    ...
    def __call__(self, noisy_w, is_ref, spk=None, z_on=True, do_prelu=False):
        ...
        with tf.variable_scope('g_ae'):
            ...
            for layer_idx, layer_depth in enumerate(segan.g_enc_depths):
                ...
                #if layer_idx == segan.att_layer_ind:
                if layer_idx in segan.enc_att_layer_ind:
                    # self-attention
                    # to 2d
                    hi_2d = tf.expand_dims(h_i, 2)
                    hi_2d = sn_non_local_block_sim(hi_2d, None, 'encoder_attention_layer{}'.format(layer_idx))
                    h_i = tf.reshape(hi_2d, hi_2d.get_shape().as_list()[:2] + [hi_2d.get_shape().as_list()[-1]])
                    print('Downconv: self-attention')
            ...
            for layer_idx, layer_depth in enumerate(g_dec_depths):
                ...
                # self-attention
                #if layer_idx == len(segan.g_enc_depths) - segan.att_layer_ind - 1: # the middle layer (5)
                if layer_idx in segan.dec_att_layer_ind:
                    # self-attention
                    # to 2d
                    hi_2d = tf.expand_dims(h_i, 2)
                    hi_2d = sn_non_local_block_sim(hi_2d, None, 'decoder_attention_layer{}'.format(layer_idx))
                    h_i = tf.reshape(hi_2d, hi_2d.get_shape().as_list()[:2] + [hi_2d.get_shape().as_list()[-1]])
                    print('Deconv: self-attention')
